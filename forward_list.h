...
#ifndef _ALGO_HIDDEN
#define _ALGO_HIDDEN __attribute__ ((__visibility__("hidden")))
#endif

#ifndef _ALGO_VIS
#define _ALGO_VIS __attribute__ ((__visibility__("default")))
#endif
...
namespace std {
    ...
    template <class T, class VoidPtr>
    struct _ALGO_HIDDEN begin_node_of {
        using type = begin_node <typename pointer_traits<VoidPtr>::template rebind<node<T, VoidPtr>>>;
    };
    ...
    class forward_list_iterator {
        ...
        template<class, class> friend class forward_list; // _ALGO_VIS
        template<class> friend class forward_list_const_iterator; // _ALGO_VIS
    ...
    public:
        ...
        friend inline bool operator==(const forward_list_iterator& x, const forward_list_iterator& y) {return x.ptr == y.ptr;}
        friend inline bool operator!=(const forward_list_iterator& x, const forward_list_iterator& y) {return x != y;} // !(x == y)
    };
    ...
    public:
        ...
        friend inline bool operator==(const forward_list_const_iterator& x, const forward_list_const_iterator& y) {return x.ptr == y.ptr;}
        friend inline bool operator!=(const forward_list_const_iterator& x, const forward_list_const_iterator& y) {return x != y;} // !(x == y)
    };
    ...
    protected:
        ...
        __compressed_pair<begin_node, node_allocator> before_begin_;
        ...
        inline node_const_pointer  __before_begin() const noexcept {
            return static_cast<node_const_pointer>(pointer_traits<begin_node_pointer>::pointer_to(const_cast<begin_node&>(before_begin_.first())));
        }

        inline node_allocator& __alloc() noexcept {return before_begin_.second();}

        inline const node_allocator& __alloc() const noexcept {return before_begin_.second();}

        using iterator = forward_list_iterator<node_pointer>;
        using const_iterator = forward_list_const_iterator<node_pointer>;

        inline forward_list_base() noexcept (is_nothrow_default_constructible<node_allocator>::value) : before_begin_(begin_node()) {}
    ...
    public:
        ...
        forward_list_base(forward_list_base&& __x, const allocator_type& __a);
    ...
    protected:
        ...
        inline void move_assign_alloc(forward_list_base& x) noexcept (!node_traits::propagate_on_container_move_assignment::value || is_nothrow_move_assignable<__node_allocator>::value) {
            move_assign_alloc(x, integral_constant<bool, node_traits::propagate_on_container_move_assignment::value>());
        }

    public:
        void swap(forward_list_base& x) noexcept (!node_traits::propagate_on_container_swap::value || __is_nothrow_swappable<node_allocator>::value);
    protected:
        void clear() noexcept;

    private:
        inline void __copy_assign_alloc(const forward_list_base&, false_type) {}

        inline void __copy_assign_alloc(const forward_list_base& x, true_type) {
            if (__alloc() != x.__alloc())
                clear();
            __alloc() = x.__alloc();
        }

        inline void __move_assign_alloc(forward_list_base& __x, false_type) noexcept {}

        inline
        void __move_assign_alloc(forward_list_base& __x, true_type) noexcept (is_nothrow_move_assignable<node_allocator>::value)
        {__alloc() = _VSTD::move(__x.__alloc());}

        inline static void __swap_alloc(node_allocator& x, node_allocator& y) noexcept (!node_traits::propagate_on_container_swap::value || is_nothrow_swappable<node_allocator>::value)
        {__swap_alloc(x, y, integral_constant<bool, node_traits::propagate_on_container_swap::value>());}

        inline static void __swap_alloc(node_allocator& __x, node_allocator& __y, false_type) noexcept {}

        inline static void __swap_alloc(node_allocator& __x, node_allocator& __y, true_type) noexcept (__is_nothrow_swappable<node_allocator>::value) {
            using _VSTD::swap;
            swap(__x, __y);
        }
    };
    ...
    public:
        ...
        forward_list() noexcept (is_nothrow_default_constructible<allocator_type>::value);
