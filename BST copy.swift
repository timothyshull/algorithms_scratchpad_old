...
//  BST
//
//  Created by Tim Shull on 11/19/16.
...
private class Node<Key:Comparable, Value> {
    ...
    private(set) public var val: Value
    private(set) public var left: Node<Key, Value>
    private(set) public var right: Node<Key, Value>
    private(set) public var size: Int

    public init(Key key, Value val, int size) {
        self.key = key;
        self.val = val;
        self.size = size;
    }
...
public class BST<Key:Comparable, Value> {
    ...
    private func getSize(node: Node<Key, Value>?) -> Int {
        if node == nil {
            return 0
        } else {
            return node!.size
    ...
    public func get(node: Node<Key, Value>?, key: Key) {
        if node == nil {
            return nil
        }
        if key < node.key {
