#ifndef CUSTOM_ALGORITHMS_SLIST_H
#define CUSTOM_ALGORITHMS_SLIST_H
...
namespace container_detail {
    ...
    struct intrusive_slist_type {
        ...
        typedef typename container_detail::bi::make_slist
                <node_type, container_detail::bi::base_hook<typename slist_hook<void_pointer>::type>, container_detail::bi::constant_time_size<true>, container_detail::bi::size_type
...
public:
    ...
    typedef BOOST_CONTAINER_IMPDEF(NodeAlloc)
...
public:
    ...
    explicit slist(const allocator_type &a) : AllocHolder(a) {}

    explicit slist(size_type n) : AllocHolder(allocator_type()) { this->resize(n); }

    slist(size_type n, const allocator_type &a) : AllocHolder(a) { this->resize(n); }
    ...
    explicit slist(size_type n, const value_type &x, const allocator_type &a = allocator_type())
            : AllocHolder(a) { this->insert_after(this->cbefore_begin(), n, x); }
    ...
    slist(InpIt first, InpIt last, const allocator_type &a = allocator_type())
            : AllocHolder(a) { this->insert_after(this->cbefore_begin(), first, last); }
    ...
    slist(std::initializer_list <value_type> il, const allocator_type &a = allocator_type())
            : AllocHolder(a) { this->insert_after(this->cbefore_begin(), il.begin(), il.end()); }
    ...
    slist(const slist &x)
            : AllocHolder(x) { this->insert_after(this->cbefore_begin(), x.begin(), x.end()); }
    ...
    slist(const slist &x, const allocator_type &a)
            : AllocHolder(a) { this->insert_after(this->cbefore_begin(), x.begin(), x.end()); }
    ...
    slist(BOOST_RV_REF(slist) x, const allocator_type &a)
        ...
        if (this->node_alloc() == x.node_alloc()) {
            ...
            this->insert_after(this->cbefore_begin(), boost::make_move_iterator(x.begin()),
    ...
    BOOST_NOEXCEPT_OR_NOTHROW {} //AllocHolder clears the slist
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { return allocator_type(this->node_alloc()); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { return this->node_alloc(); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { return iterator(end()); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { return this->cbefore_begin(); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { return iterator(this->icont().begin()); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { return this->cbegin(); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { return iterator(this->icont().end()); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { return this->cend(); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { return const_iterator(end()); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { return const_iterator(this->non_const_icont().begin()); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { return const_iterator(this->non_const_icont().end()); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { return iterator(this->icont().previous(p.get())); }
    ...
    //!
    //! <b>Note</b>: Non-standard extension.
    const_iterator previous(const_iterator p) { return const_iterator(this->icont().previous(p.get())); }
    ...
    //!
    //! <b>Complexity</b>: Constant.
    size_type size() const { return this->icont().size(); }
    ...
    //!
    //! <b>Complexity</b>: Constant.
    size_type max_size() const { return AllocHolder::max_size(); }
    ...
    template<class... Args>
    void emplace_front(BOOST_FWD_REF(Args)...
    ...
    //! <b>Complexity</b>: Constant
    template<class... Args>
    iterator emplace_after(const_iterator prev, BOOST_FWD_REF(Args)...
...
#undef BOOST_CONTAINER_SLIST_EMPLACE_CODE
    ...
    void push_front(T &&x);
#else
    BOOST_MOVE_CONVERSION_AWARE_CATCH(push_front, T,
    void, priv_push_front)
#endif
    ...
    iterator insert_after(const_iterator prev, FwdIt first, FwdIt last, typename container_detail::enable_if_c
    ...
    iterator erase_after(const_iterator before_first, const_iterator last) {
        return iterator(
                this->icont().erase_after_and_dispose(before_first.get(), last.get(), Destroyer(this->node_alloc())));
    }
    ...
    void clear() { this->icont().clear_and_dispose(Destroyer(this->node_alloc())); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { this->splice_after(prev_p, static_cast<slist &>(x)); }
    ...
    void splice_after(const_iterator prev_p, slist &x, const_iterator prev)
    ...
    //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
    //!   list. Iterators of this list and all the references are not invalidated.
    void splice_after(const_iterator prev_p, BOOST_RV_REF(slist) x, const_iterator prev)

    BOOST_NOEXCEPT_OR_NOTHROW { this->splice_after(prev_p, static_cast<slist &>(x), prev); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { this->splice_after(prev_p, static_cast<slist &>(x), before_first, before_last); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { this->splice_after(prev_p, static_cast<slist &>(x), before_first, before_last, n); }
    ...
    void remove(const T &value) { this->remove_if(equal_to_value_type(value)); }
    ...
    //! <b>Note</b>: The relative order of elements that are not removed is unchanged,
    //!   and iterators to elements that are not removed remain valid.
    void unique() { this->unique(value_equal()); }
    ...
    void merge(slist &x) { this->merge(x, value_less()); }
    ...
    //! <b>Complexity</b>: This function is linear time: it performs at most
    //!   size() + x.size() - 1 comparisons.
    void merge(BOOST_RV_REF(slist) x) { this->merge(static_cast<slist &>(x)); }
    ...
    //! <b>Note</b>: Iterators and references to *this are not invalidated.
    template<class StrictWeakOrdering>
    void merge(BOOST_RV_REF(slist) x, StrictWeakOrdering comp) { this->merge(static_cast<slist &>(x), comp); }
    ...
    //!   is the list's size.
    void sort() { this->sort(value_less()); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { this->icont().reverse(); }
    ...
    template<class... Args>
    iterator emplace(const_iterator p, BOOST_FWD_REF(Args)...
...
#undef BOOST_CONTAINER_SLIST_EMPLACE_CODE
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { return iterator(this->erase_after(previous(p))); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { return iterator(this->erase_after(previous(first), last)); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { this->splice_after(this->previous(p), x); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { this->splice(p, static_cast<slist &>(x)); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { this->splice_after(this->previous(p), x, this->previous(i)); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { this->splice(p, static_cast<slist &>(x), i); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { this->splice_after(this->previous(p), x, this->previous(first), this->previous(last)); }
    ...
    BOOST_NOEXCEPT_OR_NOTHROW { this->splice(p, static_cast<slist &>(x), first, last); }
    ...
    //!
    //! <b>Complexity</b>: Linear to the number of elements in the container.
    friend bool operator!=(const slist &x, const slist &y) { return !(x == y); }
    ...
    //!
    //! <b>Complexity</b>: Linear to the number of elements in the container.
    friend bool operator>(const slist &x, const slist &y) { return y < x; }
    ...
    //!
    //! <b>Complexity</b>: Constant.
    friend void swap(slist &x, slist &y) { x.swap(y); }

#ifndef BOOST_CONTAINER_DOXYGEN_INVOKED
private:

    void priv_push_front(const T &x) { this->insert_after(this->cbefore_begin(), x); }

    void priv_push_front(BOOST_RV_REF(T) x) { this->insert_after(this->cbefore_begin(), ::boost::move(x)); }
    ...
    public:
        ...
        iiterator inserted_first() const { return ret_; }
    };
    ...
    struct value_equal_to_this {
        ...
        bool operator()(const value_type &val) const { return m_ref == val; }
...
#endif //CUSTOM_ALGORITHMS_SLIST_H
